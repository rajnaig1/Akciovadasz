...
@section('content')
        ...
        @foreach ($shoppingCarts as $shoppingCart)
            @if ($shoppingCart->shop == 'Penny' && $shoppingCart->product_id != null)
                <tr class="text-center">
                    ...
                    <td><button class="btn btn-primary modalToggler" data-bs-toggle="modal" id="{{ $shoppingCart->id }}"
                            data-bs-target="#editModal">Szerkesztés</button></td>
                    <td><button class="btn btn-danger modalDeleteToggler" data-bs-toggle="modal"
                            id="{{ $shoppingCart->id }}" data-bs-target="#deleteModal">Törlés</button></td>
                </tr>
            @elseif ($shoppingCart->shop == 'Penny' && $shoppingCart->product_id == null)
                <tr class="text-center">
                    @include('../shoppingcart.shoppingcart_custom_tablecontents')
                    <td><button class="btn btn-primary modalToggler" data-bs-toggle="modal" id="{{ $shoppingCart->id }}"
                            data-bs-target="#editCustomModal">Szerkesztés</button></td>
                    <td><button class="btn btn-danger modalDeleteToggler" data-bs-toggle="modal"
                            id="{{ $shoppingCart->id }}" data-bs-target="#deleteModal">Törlés</button></td>
                </tr>
            @endif
    ...
    <x-shopping-cart-table-component h2="Tesco" id="Tesco">
        @foreach ($shoppingCarts as $shoppingCart)
            @if ($shoppingCart->shop == 'Tesco' && $shoppingCart->product_id != null)
                <tr class="text-center">
                    @include('../shoppingcart.shoppingcart_tablecontents')
                    <td><button class="btn btn-primary modalToggler" id="{{ $shoppingCart->id }}" data-bs-toggle="modal"
                            data-bs-target="#editModal">Szerkesztés</button></td>
                    <td><button class="btn btn-danger modalDeleteToggler" data-bs-toggle="modal"
                            id="{{ $shoppingCart->id }}" data-bs-target="#deleteModal">Törlés</button></td>
    ...
    <x-login-register-component class="nav-link-active" buttonText="Szerkesztés" id="editModal" title="Termék szerkesztése"
        body="" url="/updateshoppingcart" method="POST">
        @method('PUT')
        <div class="text-center d-grid gap-2">
            ...
            <input type="number" id="modalEditAmount" class="form-control" name="amount" placeholder="" value=""
                required>
            ...
            <input type="textarea" class="form-control" id="modalEditComment"name="comment" placeholder="Megjegyzés"
                value="">
        </div>
    </x-login-register-component>
    <x-login-register-component class="nav-link-active" buttonText="Szerkesztés" id="editCustomModal"
        title="Termék szerkesztése" body="" url="/updatecustomshoppingcart" method="POST">
        @method('PUT')
        <div class="text-center d-grid gap-2">
            ...
            <input type="number" id="modalEditCustomAmount" class="form-control" name="amount" placeholder=""
                value="" required>
            ...
            <input type="textarea" class="form-control" id="modalEditCustomComment"name="comment"
                placeholder="Megjegyzés" value="">
        </div>
    </x-login-register-component>
    <x-login-register-component class="nav-link-active" buttonText="Törlés" id="deleteModal" title="Termék törlése"
        body="Biztosan törölni akarja a terméket?" url="/deleteshoppingcart" method="POST">
        @method('DELETE')
        <p id="modalDeleteName"></p>
        <input type="text" id="modalDeleteId" name="id" hidden>
    </x-login-register-component>
    <x-login-register-component class="nav-link-active" buttonText="Felvétel" id="customModal"
        title="Egyéb Termék felvétele" body="" url="/addcustomshoppingcart" method="POST">
        <div class="text-center d-grid gap-2">
            ...
            <input type="number" placeholder="Mennyiség" id="modalEditAmount" class="form-control" name="amount"
