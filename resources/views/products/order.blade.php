...
<form action="/addtoshoppingcart" method="POST">
    ...
    <input type="number" name="amount" placeholder="Mennyiség" id="{{ $product->id.'shoppingCartamount' }}" required style="display:none">
    <select name="quantity" id="{{ $product->id.'shoppingCartunit' }}"style="display:none">
        <option value="db">Darab</option>
        <option value="kg">Kilogram</option>
        <option value="l">Liter</option>
    </select>
    <input type="textarea" name="comment" placeholder="Megjegyzés..." id="{{ $product->id.'shoppingCartcomment' }}"style="display:none">
    <button class='btn btn-success' id="{{ $product->id.'shoppingCartsubmit' }}" type="submit" style="display:none">Felír</button>
